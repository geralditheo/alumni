...
export function useAnnouncement(){
    ...
    const index = async (): Promise< Announcement[] | void> => {
        try {
            ...
            const { data } = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/pengumuman` , {
                headers: {
    ...
    const post = async (formData: FormData): Promise<void> => {
        try {
            ...
            const { data } = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/pengumuman` , formData, {
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            });
    ...
    const show = async (uuid: string): Promise< Announcement | undefined > => {
        try {
            ...
            const { data } = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/pengumuman/${uuid}` , {
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            });
    ...
    const update = async (formData: FormData, uuid: string): Promise<void> => {
        try {
            ...
            const { data } = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/pengumuman/${uuid}` , formData, {
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            });
    ...
    const remove = async (uuid: string): Promise< Announcement | undefined > => {
        try {
            ...
            const { data } = await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/pengumuman/${uuid}` , {
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            });
