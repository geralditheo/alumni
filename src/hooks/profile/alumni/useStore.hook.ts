...
export function useProfile(){
    ...
    const changePasswordAlumni = async (data: FormData): Promise<void> => {
        try {
    ...
    const changePasswordAdmin = async (data: FormData): Promise<void> => {
        try {
            const response = await axios.patch(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/admin/profile/update-password`, data, {
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            });
